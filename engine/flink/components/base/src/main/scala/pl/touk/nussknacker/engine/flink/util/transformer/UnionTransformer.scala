      ...
      .implementedBy(
        new FlinkCustomJoinTransformation {
          override def transform(inputs: Map[String, DataStream[Context]], context: FlinkCustomNodeContext): DataStream[ValueWithContext[AnyRef]] = {
            implicit val ti = TypeInformation.of(classOf[ValueWithContext[AnyRef]])
            ...
            val connectedStream = valuesWithContexts.reduce(_.connect(_).map(new CoMapFunction[ValueWithContext[AnyRef], ValueWithContext[AnyRef], ValueWithContext[AnyRef]] {
