              ...
              complete {
                processRepository.fetchLatestProcessDetailsForProcessId[CanonicalProcess](processId.id).map[ToResponseMarshallable] {
                  ...
                  case Some(process) => {
                    ...
                    val processConfig    = processTypeData.collect(_.modelData).collect(_.processConfig).get
                    val classLoader = processTypeData.collect(_.modelData).collect(_.modelClassLoader).collect(_.classLoader).get
