...
class MethodTypeInfoSubclassCheckerSpec extends AnyFunSuite with Matchers with ValidatedValuesDetailedMessage {
  ...
  test("should work when only subclass has varArgs") {
    val table = Table(
      ...
      (List(), Some(Typed[String]), List(Typed[String], Typed[String]), None, List(BadVarArg, WrongNumberOfArguments(0, 2))),
      (List(Typed[Int]), Some(Typed[Int]), List(Typed[Int]), None, List(BadVarArg)),
      (List(Typed[Int]), Some(Typed[String]), List(Typed[Int], Typed[Int]), None, List(BadVarArg, WrongNumberOfArguments(1, 2)))
    )
    forAll(table)(check)
  }
  ...
  test("should work when only superclass has varArgs") {
    val table = Table(
      ...
      (List(Typed[Int], Typed[String], Typed[Double]), None, List(), Some(Typed[Number]), List(NotSubclassArgument(2, Typed[String], Typed[Number])))
    )
    forAll(table)(check)
  }
  ...
  test("should work when subclass and superclass have varArgs") {
    val table = Table(
      ...
      (List(Typed[Int]), Some(Typed[String]), List(Typed[Int], Typed[String]), Some(Typed[String]), List(NotEnoughArguments(1, 2))),
